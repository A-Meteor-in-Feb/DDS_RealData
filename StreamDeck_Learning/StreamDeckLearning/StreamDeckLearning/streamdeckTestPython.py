...
IMAGE_FILES = {
    'P_select': 'images/1_P_select.png',
    'P_unselect': 'images/1_P_unselect.png',
    'R_select': 'images/2_R_select.png',
    'R_unselect': 'images/2_R_unselect.png',
    'N_select': 'images/3_N_select.png',
    'N_unselect': 'images/3_N_unselect.png',
    'D_select': 'images/4_D_select.png',
    'D_unselect': 'images/4_D_unselect.png',
...
def read_streamdeck():
    ...
    print(f"No StreamDeck found with serial number {DESIRED_SERIAL_NUMBER}.")
    return None

def set_button_image(deck, button, state):
    icon_path = IMAGE_FILES[f"{GEAR_BUTTONS[button]}_{state}"]
...
def send_initial_button_states(socket):
    while True:
        try:
            socket.send_string(str(button_states), zmq.NOBLOCK)
            break
        except zmq.Again:
            print("Sending initial button states failed, retrying...")
            time.sleep(1)
...
def main():
    ...
    pair_socket.connect("tcp://localhost:5555")
    pair_socket.setsockopt(zmq.RCVBUF, 0)
    ...
    try:
        while True:
            ...
            try:
                message = pair_socket.recv_string(zmq.NOBLOCK)
                ...
                if not initialize:
                    ...
                    initialize = True
                height, depth, auto_flag = message.split()
                ...
                print(f"Exception during message reception: {e}")
            ...
            if height != last_height:
                show_text(streamdeck, 9, f"Height:\n{height}m", 22)
                last_height = height
            if depth != last_depth:
                show_text(streamdeck, 14, f"Depth:\n{depth}m", 22)
                last_depth = depth
